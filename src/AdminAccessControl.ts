...
async function checkLogin(checkExpiringSoon: boolean = false): Promise<boolean> {
  try {
    const pluginLocalAuthStr = await sdk.storage.getItem('plugin-admin-local-auth');
...
export async function clearAuth(): Promise<void> {
  try {
    await sdk.storage.removeItem('zadig-admin-auth-config');
    await sdk.storage.removeItem('plugin-admin-local-auth');
  } catch (error) {
...
async function getAuthConfig() {
  try {
    const config = await sdk.storage.getItem('zadig-admin-auth-config');
